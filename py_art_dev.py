...
# Commandline args:
if len(sys.argv) != 4:
	print("Usage: ./py_art_dev.py <text> <mirror> <square_scale>")
...
while ((draw.textbbox((0, 0), text, font=font)[2] <= (width-font_offset)) and (font_size <= font_max_size)):
	font_size += 1;
...
for i in range(0,r_width,sqaure_scale):
	for j in range(0,height,sqaure_scale):
		...
		for k in range(1,sqaure_scale, sqaure_scale//10):
			...
			safe_rectangle([(i+sqaure_scale//2+(mod1*k), j+sqaure_scale//2+(mod2*k)), (i+sqaure_scale//2+(mod3*k)-mini_scale, j)], c2)
